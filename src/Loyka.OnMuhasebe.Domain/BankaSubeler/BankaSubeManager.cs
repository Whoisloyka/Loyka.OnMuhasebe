...
public class BankaSubeManager : DomainService
{
    ...
    public BankaSubeManager(IBankaSubeRepository bankaSubeRepository, IBankaRepository bankaRepository, IOzelKodRepository ozelKodRepository)
    {
    ...
    // burada yazacağımız functionların sabit id alanları olduğu için ayrı bir Interface yazmak pek kullanışlı değil. O yüzden doğrudan burada yazacağız. 
    ...
    public async Task CheckUpdateAsync(Guid id, string kod, BankaSube entity, Guid? ozelKod1Id, Guid? ozelKod2Id)
    {
        await _bankaSubeRepository.KodAnyAsync(kod,
            ...
            entity.Kod!=kod); // await ile kod kontroll
        await _ozelKodRepository.EntityAnyAsync(ozelKod1Id, OzelKodTuru.OzelKod1,
    ...
    {
        await _bankaSubeRepository.RelationalEntityAsync(
